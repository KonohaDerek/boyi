...
func (s *service) UpdateRole(ctx context.Context, opt *option.RoleWhereOption, col *option.RoleUpdateColumn) error {
	var (
		userRoles []dto.MerchantUserRole
		roles     []dto.Role
	)
	...
	_, err = s.repo.List(ctx, nil, &userRoles, &option.MerchantUserRoleWhereOption{
		RoleIDs: tmp,
...
func (s *service) ClearUserRolesClaims(ctx context.Context, userRoles []dto.MerchantUserRole) error {
	var (
	...
	_, err := s.repo.List(ctx, nil, &users, &option.MerchantUserWhereOption{
		IDs: userIds,
	})
	if err != nil {
		return err
