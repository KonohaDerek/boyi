...
func New(config *Config) (IPSearch, error) {
	path := os.Getenv("PROJ_DIR") + config.FilePath
	if ips == nil {
		var err error
		ips, err = loadIpDat(path)
		if err != nil {
			log.Fatal().Msg("the IP Dat loaded failed!")
			return *ips, err
		}
	}
	return *ips, nil
...
func loadIpDat(filePath string) (*ipSearch, error) {

	p := ipSearch{}
	//加载ip地址库信息
	data, err := ioutil.ReadFile(filePath)
	if err != nil {
		log.Err(err).Msg("load response body has failed")
		return nil, err
	}
	p.data = data
